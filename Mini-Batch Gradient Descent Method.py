...
def mini_batch_gradient_descent(X, y, lr=0.01, epochs=1000, batch_size=32):
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, m, batch_size):
            ...
            d_theta = np.dot(X_batch.T, error)
            d_bias = np.sum(error)

            theta -= lr * d_theta / batch_size
            bias -= lr * d_bias / batch_size
...
theta, bias, losses = mini_batch_gradient_descent(X_train, y_train)
...
plt.title("Mini-Batch Gradient Descent: Predicted vs Actual")
plt.xlabel("Actual")
...
plt.title("Mini-Batch Gradient Descent: Performance Metrics")
plt.ylabel("Value")
plt.show()
